<p>
    <label for="<?php echo $data['widget']->get_field_id('title' . $i); ?>"><?php esc_html_e('Title:', 'wbfy-icon-grid-it');?></label>
    <input
        type="text"
...
<div>
    <select
        ...
        title="<?php esc_html_e('Title alignment', 'wbfy-icon-grid-it');?>"
        name="<?php echo $data['widget']->get_field_name('title_align'); ?>"
    >
        <option value="left"<?php echo ($data['content']['title_align'] == 'left') ? ' selected' : ''; ?> >
            <?php echo esc_html_e('Aligned Left', 'wbfy-icon-grid-it'); ?>
        </option>
        <option value="right"<?php echo ($data['content']['title_align'] == 'right') ? ' selected' : ''; ?> >
            <?php echo esc_html_e('Aligned Right', 'wbfy-icon-grid-it'); ?>
        </option>
        <option value="center"<?php echo ($data['content']['title_align'] == 'center') ? ' selected' : ''; ?> >
            <?php echo esc_html_e('Centered', 'wbfy-icon-grid-it'); ?>
        </option>
    </select>
</div>
<p>
    <label for="<?php echo $data['widget']->get_field_id('icon_color' . $i); ?>"><?php esc_html_e('Icon Color:', 'wbfy-icon-grid-it');?></label>
    <input
        type="text"
...
for ($i = 1; $i <= WBFY_IGI_MAX_ITEMS; $i++) {
    ?>
<p>
    <label for="<?php echo $data['widget']->get_field_id('item' . $i); ?>"><?php esc_html_e('Item ' . $i . ':', 'wbfy-icon-grid-it');?></label>
    <input
        type="text"
        placeholder="<?php esc_html_e('Icon', 'wbfy-icon-grid-it');?>"
...
<div>
    <input
        type="text"
        placeholder="<?php esc_html_e('Text', 'wbfy-icon-grid-it');?>"
